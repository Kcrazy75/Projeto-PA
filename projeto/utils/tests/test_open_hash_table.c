...
void setUp() {
    table = hash_table_create(-1, NULL, NULL, NULL, NULL);
}
...
void test_custom_table() {
    hash_table_destroy(table);
    table = hash_table_create(10, custom_hash, custom_key_equal, custom_key_destroy, custom_value_destroy);
...
void test_update() {
    ...
    hash_table_update(table, "key3", new_value);
    TEST_ASSERT_EQUAL_STRING(new_value, hash_table_get(table, "key3"));
}

void test_rehash() {
    hash_table_destroy(table);
    table = hash_table_create(3, NULL, NULL, NULL, NULL);
    hash_table_insert(table, keys[0], values[0]);
